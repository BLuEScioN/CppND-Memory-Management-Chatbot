...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes
    // deallocation handled by smartpointers
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // GraphNode holds non-owning references to incoming GraphEdges
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // GraphNode exclusively owns the outgoing GraphEdges 
                        }
    ...
    SetChatbotHandle(&chatBot); // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
